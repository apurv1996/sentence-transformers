...
class BatchPosTripletLoss(nn.Module):
    """
    BatchPosTripletLoss takes a batch with (label, sentence) pairs and computes the loss for all possible, valid
    ...
    Example::
        ...
        train_loss = losses.BatchPosTripletLoss(model=model)
    """

    def __init__(self, model: SentenceTransformer,
                 distance_metric=BatchHardTripletLossDistanceFunction.eucledian_distance, margin: float = 5):
        super(BatchPosTripletLoss, self).__init__()
    ...
    def batch_all_triplet_loss(self, labels, embeddings):
        ...
        pos_mask = (labels == 1).unsqueeze(-1).unsqueeze(-1) & mask
        triplet_loss = pos_mask.float() * triplet_loss
